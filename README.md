...
With compiler context, Nirvana builds declaration-resolved function summaries, closes direct internal-call effects, and compares named inverse pairs such as `borrow/repay`, `deposit/withdraw`, `mint/burn`, `stake/unstake`, and `grant/revoke`. It emits separate hypotheses for asymmetric accounting writes, security guards, and security-sensitive effects. Guard comparison is directional for measured value-flow pairs: expected owner/solvency gates on withdrawal, redemption, removal, or debt creation are not treated as missing parity, while reverse-direction and pause, reentrancy, deadline, or nonce differences remain review candidates. The operation-summary contract is language-neutral, but the bundled production frontend is currently the typed solc AST frontend. Syntax-only SSG slices do not emit inverse-parity hypotheses; a future Move, Rust, Solana, JVM, or Web/API frontend must supply typed state, guard, and effect facts before using the same analyzer. The pinned ten-repository measurement now records zero baseline false positives plus 7/10 detected semantic mutations, with every injection reached and triggered; the exact cases and all limitations are published in [the August 2026 symmetry study](docs/symmetry-study-2026-08.md).
...
Closed beta separates the PDF's 80% overall and 90% High/Critical precision targets from `nirvana-closed-beta-v8` suite qualification. A passing suite also needs at least 20 eligible vulnerable cases, 10 eligible benign controls, three distinct seeds per eligible case, a verified independently authored encrypted case pack, a post-trial reveal matching every pre-trial eligibility/class/label, KLOC, disclosure, and hidden-variant commitment, finding IDs bound to that committed ground truth, and every trial bound to the case pack's exact canonical intake target snapshot. The complete finding set must match a sealed hash-chained ledger checkpoint with replay-derived reproduction, regression, novelty, duplicate, timing, six-dimension coverage state, and a hash-verified execution-receipt runtime floor. Magma levels must be monotonic with detections derived from attributed true positives and triggers including replay-demonstrated conditions. `magma.reach_source` remains explicitly `declared`; the other level sources are published beside it. The suite also requires executable-or-stronger evidence for every High/Critical report, complete novelty adjudication, and complete trial cost accounting with an ordered time window, positive worker count, and declared compute between the checkpointed receipt-duration floor and wall-clock hours times workers. A checkpointed finding cannot be suppressed from precision adjudication. A clean smaller pilot remains valid and keeps its supported metrics, but per-KLOC yield is undefined without an independent size commitment, and novelty yield or duplicate rate is undefined when the relevant checkpointed findings lack novelty assessments. A contaminated manifest, commitment or audited-target mismatch, invalid finding attribution, broken ledger binding, understated receipt runtime, or contradictory Magma declaration produces `valid: false`, records every violation in `invalid_reasons`, writes `metrics: null` and `magma: null`, forces every release gate closed, prints the violations, and exits `2`. Without `--output`, the report is written beside the manifest rather than into the caller's current directory. Undefined metrics in a valid report stay undefined rather than being reported as zero. See [blind benchmark operations](docs/benchmarking.md) for case-pack custody, pre-reveal trial sealing, and the pinned executable harness; the sendable five-case pilot specification is in [the independent case-pack commission](docs/independent-case-pack-commission.md).
