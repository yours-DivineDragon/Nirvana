...
With compiler context, Nirvana builds declaration-resolved function summaries, closes direct internal-call effects, and compares named inverse pairs such as `borrow/repay`, `deposit/withdraw`, `mint/burn`, `stake/unstake`, and `grant/revoke`. It emits separate hypotheses for asymmetric accounting writes, security guards, and security-sensitive effects. The operation-summary contract is language-neutral, but the bundled production frontend is currently the typed solc AST frontend. Syntax-only SSG slices do not emit inverse-parity hypotheses; a future Move, Rust, Solana, JVM, or Web/API frontend must supply typed state, guard, and effect facts before using the same analyzer.
