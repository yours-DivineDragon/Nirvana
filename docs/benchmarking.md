...
| Case provenance | Verified independent encrypted case pack |
...
The case-pack author and trial operator must be separated by the declared custody model. Before the trial, the author:

1. Pins a cutoff and creates post-cutoff targets without exposing vulnerable-versus-benign labels.
2. Encrypts each ground-truth document for a recipient unavailable to the trial operator. `age` and OpenPGP are the supported contract labels.
3. Retains an authorship/custody attestation, generator source hash and randomness description.
4. Hashes each target snapshot, public commitment, ciphertext, and attestation in `benchmark-case-pack.json`.
...
The verifier rejects path traversal, symlinks, non-regular artifacts, oversized inputs, target snapshot drift, duplicate ids, pre-cutoff cases, and every hash mismatch. It proves byte integrity and the declared custody contract; it cannot prove that the author was honest or that encryption keys were kept separate. Those remain independent operational controls.
...
Evaluation re-verifies the pack instead of trusting a prior report. The pack cutoff, complete case-id set, and case origin timestamps must exactly match the manifest for suite qualification.
...
Temporal contamination still invalidates the entire report: metrics and Magma values become null, every gate closes, and the CLI exits non-zero. Sample insufficiency does not invalidate a clean pilot; it appears in `suite_qualification.reasons` instead.
