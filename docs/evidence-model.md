...
Blind benchmark runs add a typed `benchmark_trial_sealed` boundary before reveal. The seal is derived from the ledger-backed scope and full confirmed-finding set and binds trial, case, run, seed, canonical intake target algorithm and digest, severity, evidence tier, reproduction/regression state, novelty classification and corpus hash, exact-duplicate/novel decisions, ledger-derived time-to-finding, the canonical six-dimension coverage projection reconstructed from the initial schedule and coverage events, and a hash-verified aggregate of every distinct checkpointed execution receipt's target and negative-control duration. `nirvana benchmark seal-trial` appends it only after the run is complete and every supporting evidence record is replay-verified, then exports the checkpoint named by the benchmark manifest. Evaluation rechecks the seal, receipt artifacts and replay links, the checkpoint prefix, the current ledger's later replay status, the independently committed case-pack target, coverage schedule and events, a one-to-one manifest finding set, and that declared compute is at least the receipt-derived runtime floor. The seal does not replace independent publication of the checkpoint hash, and the floor does not measure unrecorded agent-host overhead.
