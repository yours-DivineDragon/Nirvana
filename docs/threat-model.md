...
| Benchmark contamination, target substitution, post-trial relabelling/suppression, metric-input or Magma-level inflation, invented/omitted findings, or one-case overclaim | Cutoff checks across training/retrieval/rules, blocked ground truth and eventual fixes, independent encrypted case-pack hashes, exact canonical intake target equality between pack and sealed run, canonical eligibility/class/label commitments, committed KLOC/disclosure/variant metadata, ordered trial windows with positive worker-bounded compute, mandatory adjudication of every confirmed finding, pre-reveal trial seals with ledger-derived reproduction/regression, novelty/duplicate, timing, and six-dimension coverage state, ledger-derived detection, trigger lower bounds, Magma monotonicity, hash-verified checkpoints, repeat seeds, benign controls, and archived trial provenance |
