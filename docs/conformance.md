...
| Temporal evaluation | Blind cutoff validation, hidden-variant logs, independent encrypted case-pack verification, canonical reveal and KLOC commitments, committed finding attribution, sealed run-ledger checkpoints, and archived model/prompt/tool/budget/environment/transcript provenance | benchmark manifest v1.3, trial seal v1.2, case pack/report v1.2, benchmark report v1.7 | Temporal, pack/manifest, reveal, sizing, attribution, or trial-ledger mismatches set report-level `valid: false`; artifact metrics and Magma results are null, all release gates close, and exit status is nonzero |
| Metrics and release gates | Eleven source metrics, Magma reached/triggered/detected, plus research, Web3-alpha, closed-beta, production, and universal-expansion gates backed by explicit release evidence | `benchmark-report.json` | Valid pilot metrics are distinct from maturity: closed beta uses the published v4 case-count, benign-control, repeat-seed, independence, committed-reveal/size, ledger-derived coverage/outcomes/timing/novelty, actionable-evidence, and accounting floors |
