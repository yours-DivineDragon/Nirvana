...
| Candidate ensemble | Lexical, EVM AST, typed-solc symmetry, graph query, attacker sequence, spec inference, test gap, historical variant, retrieval analogy, and differential mismatch | `hypothesis_proposed.generator` | Generators optimize recall and produce no proof; inverse parity currently requires the typed solc frontend, and one provenance template yields one historical or retrieval candidate rather than a mirrored pair |
...
| Efficiency/model routing | Per-hypothesis model cost fields and execution duration accounting; declared benchmark compute is bounded below by hash-verified checkpointed receipt runtime and above by the ordered wall-clock window times a positive worker count; no model APIs | report `cost_accounting`, benchmark metrics | Agent-side token/cost/worker data must be supplied by the agent host; receipt runtime covers observed verifier/control work but not unrecorded host overhead |
| Temporal evaluation | Blind cutoff validation, hidden-variant logs, independent encrypted case-pack verification, canonical reveal and independently committed target/size/disclosure/variant metadata, committed finding attribution, sealed run-ledger checkpoints, and archived model/prompt/tool/budget/environment/transcript provenance | benchmark manifest v1.5, trial seal v1.4, case pack/report v1.4, benchmark report v1.11 | Temporal, pack/manifest, reveal, target, sizing/metadata, attribution, trial-ledger/receipt-runtime, or Magma-binding mismatches set report-level `valid: false`; artifact metrics and Magma results are null, all release gates close, and exit status is nonzero |
| Metrics and release gates | Eleven source metrics; declared reach, ledger-lower-bounded trigger, and ledger-derived detection with Magma monotonicity; plus research, Web3-alpha, closed-beta, production, and universal-expansion gates backed by explicit release evidence | `benchmark-report.json` | Valid pilot metrics are distinct from maturity: closed beta uses the published v8 case-count, benign-control, repeat-seed, independence, committed reveal/target/size/case metadata, ledger-derived coverage/outcomes/timing/novelty/detection, monotonic Magma, actionable evidence, and receipt-to-worker-bounded accounting floors |
| Ledger anchoring | Hash chain plus portable prefix checkpoints that remain valid after later appends; benchmark trial seals bind run/case/seed, the canonical intake target, the complete typed finding set, derived reproduction/regression/novelty/timing state, reconstructed six-dimension coverage, and the distinct execution-receipt runtime aggregate to a checkpoint | `ledger-checkpoint.json`, `benchmark_trial_sealed` | Publishing the checkpoint to an independent trusted store is deployment work |
