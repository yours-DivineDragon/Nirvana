...
| Temporal evaluation | Blind cutoff validation, hidden-variant logs, independent encrypted case-pack verification, canonical reveal commitments, and archived model/prompt/tool/budget/environment/transcript provenance | benchmark manifest v1.0, case pack/report v1.1, benchmark report v1.4 | Temporal, pack/manifest, or reveal mismatches set report-level `valid: false`; artifact metrics and Magma results are null, all release gates close, and exit status is nonzero |
