...
| Efficiency/model routing | Per-hypothesis model cost fields and execution duration accounting; declared benchmark compute is positive and bounded by the ordered wall-clock window times a positive worker count; no model APIs | report `cost_accounting`, benchmark metrics | Agent-side token/cost/worker data must be supplied by the agent host; the bounds establish consistency, not host honesty |
| Temporal evaluation | Blind cutoff validation, hidden-variant logs, independent encrypted case-pack verification, canonical reveal and independently committed target/size/disclosure/variant metadata, committed finding attribution, sealed run-ledger checkpoints, and archived model/prompt/tool/budget/environment/transcript provenance | benchmark manifest v1.5, trial seal v1.3, case pack/report v1.4, benchmark report v1.10 | Temporal, pack/manifest, reveal, target, sizing/metadata, attribution, trial-ledger, or Magma-binding mismatches set report-level `valid: false`; artifact metrics and Magma results are null, all release gates close, and exit status is nonzero |
| Metrics and release gates | Eleven source metrics; declared reach, ledger-lower-bounded trigger, and ledger-derived detection with Magma monotonicity; plus research, Web3-alpha, closed-beta, production, and universal-expansion gates backed by explicit release evidence | `benchmark-report.json` | Valid pilot metrics are distinct from maturity: closed beta uses the published v7 case-count, benign-control, repeat-seed, independence, committed reveal/target/size/case metadata, ledger-derived coverage/outcomes/timing/novelty/detection, monotonic Magma, actionable-evidence, and worker-bounded accounting floors |
| Ledger anchoring | Hash chain plus portable prefix checkpoints that remain valid after later appends; benchmark trial seals bind run/case/seed, the canonical intake target, the complete typed finding set, derived reproduction/regression/novelty/timing state, and reconstructed six-dimension coverage to a checkpoint | `ledger-checkpoint.json`, `benchmark_trial_sealed` | Publishing the checkpoint to an independent trusted store is deployment work |
