...
| Temporal evaluation | Blind cutoff validation, hidden-variant logs, independent encrypted case-pack verification, canonical reveal and independently committed size/disclosure/variant metadata, committed finding attribution, sealed run-ledger checkpoints, and archived model/prompt/tool/budget/environment/transcript provenance | benchmark manifest v1.4, trial seal v1.2, case pack/report v1.3, benchmark report v1.8 | Temporal, pack/manifest, reveal, sizing/metadata, attribution, trial-ledger, or Magma-binding mismatches set report-level `valid: false`; artifact metrics and Magma results are null, all release gates close, and exit status is nonzero |
| Metrics and release gates | Eleven source metrics; declared reach, ledger-lower-bounded trigger, and ledger-derived detection with Magma monotonicity; plus research, Web3-alpha, closed-beta, production, and universal-expansion gates backed by explicit release evidence | `benchmark-report.json` | Valid pilot metrics are distinct from maturity: closed beta uses the published v5 case-count, benign-control, repeat-seed, independence, committed reveal/size/case metadata, ledger-derived coverage/outcomes/timing/novelty/detection, monotonic Magma, actionable-evidence, and accounting floors |
| Ledger anchoring | Hash chain plus portable prefix checkpoints that remain valid after later appends; benchmark trial seals bind run/case/seed, the complete typed finding set, derived reproduction/regression/novelty/timing state, and reconstructed six-dimension coverage to a checkpoint | `ledger-checkpoint.json`, `benchmark_trial_sealed` | Publishing the checkpoint to an independent trusted store is deployment work |
