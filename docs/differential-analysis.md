...
Report validity is fail-closed at the harness level. `scheduled_executions` is the full case × implementation × repetition budget; `successful_executions` counts unblocked, non-timeout runs whose output the comparator can normalize, while `unnormalizable_executions` completes the accounting for deterministic rejection output. A case for which no implementation normalizes is recorded in `unnormalizable_cases`, excluded from the comparison denominator, and does not invalidate a run that still has comparable executions. If only some implementations normalize, the case is a mismatch. Blocked, timed-out, flaky, zero-normalizable, or incompletely accounted schedules set `valid` false; `invalid_reasons` explains why, the CLI prints `INVALID` and exits `2`, and mismatch/agreement counts are diagnostic only. A non-zero return remains a valid observable outcome when its output is normalizable. On import, Nirvana independently checks case arithmetic, schedule coverage, outcome membership, repetitions, case-level rejection records, reasons, and derived validity. Flipping a serialized `valid` flag cannot unlock attach, classification, or disclosure.
