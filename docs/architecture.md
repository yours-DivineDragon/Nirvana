...
The solc frontend resolves state declarations and follows direct internal function calls to a fixed point before comparison. It is currently the only bundled frontend allowed to emit inverse-parity hypotheses. Syntax-only SSG slices do not populate declaration-resolved state, guard, and effect facts, so they are deliberately excluded. The shared representation makes future typed Move, Rust, Solana, JVM, or Web/API frontends additive rather than requiring a new detector.
